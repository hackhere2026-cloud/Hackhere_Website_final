...
export default function UserDashboard() {
  ...
  useEffect(() => watchFirebaseUser((nextUser) => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // Firebase OAuth profiles are immediately usable even when the legacy
        // Supabase profile database is not connected.
        if (!user && firebaseUser) {
          setUserData({
            displayName: firebaseUser.displayName || firebaseUser.email?.split("@")[0] || "Builder",
            email: firebaseUser.email,
  ...
  useEffect(() => {
    if (!isAppLoading && firebaseResolved && !activeUser) {
      navigate('/');
    }
  }, [activeUser, firebaseResolved, isAppLoading, navigate]);
  ...
  return (
    <div className="min-h-screen bg-[#EAE6DF] text-[#080B10] font-sans p-4 sm:p-8 lg:p-12 selection:bg-[#080B10] selection:text-[#61C8D4] pt-32">
      ...
      <div className="max-w-[1400px] mx-auto">
        ...
        {/* Navbar / Header area */}
        <div className="flex justify-between items-end mb-10 border-b border-[#E2E8F0] pb-6">
          <div>
            <span className="text-xs font-mono uppercase tracking-[0.25em] text-[#FF2D5D] font-bold">
              MY HACKHERE
            </span>
            <h1 className="text-4xl md:text-6xl font-serif font-light tracking-tight mt-1 text-[#080B10]">
              BUILDER DASHBOARD
            </h1>
            <p className="text-sm md:text-base font-light text-[#4A5568] mt-1 font-sans">
              Welcome back, {userData?.displayName?.split(' ')[0] || "Creator"}
            </p>
          </div>
        </div>
